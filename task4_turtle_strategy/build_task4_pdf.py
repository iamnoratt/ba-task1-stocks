...
REPORT_MD = BASE_DIR / "report" / "task4_report_final.md"
OUTPUT_PDF = BASE_DIR / "report" / "诺拉+TASK4.pdf"
...
def format_inline_markdown(text: str) -> str:
    ...
    def stash(match: re.Match[str]) -> str:
        ...
        placeholders[token] = escape_html(match.group(1))
        return token
    ...
    for token, value in placeholders.items():
        text = text.replace(token, f"<font name='Courier'>{value}</font>")
    return text
...
def build_story():
    ...
    for raw_line in lines:
        ...
        if line.startswith("## "):
            story.append(Paragraph(format_inline_markdown(line[3:]), h1_style))
            continue

        if line.startswith("### "):
            story.append(Paragraph(format_inline_markdown(line[4:]), h2_style))
            continue

        if line.startswith("- "):
            story.append(Paragraph(format_inline_markdown("• " + line[2:]), bullet_style))
            continue
